...
main = do
  ...
  case args of
    ["fast"] -> mainFast
    ["pretty"] -> mainPretty
    _ -> putStrLn "Please run benchmark with either 'fast' or 'pretty' argument."
...
mainFast :: IO ()
mainFast = withTrailDB "/mnt/data/wikipedia-history-small.tdb" $ \traildb -> do
  ...
  print =<< foldTrailDB (countEditsAndIPs cursor (fromIntegral user_field) (fromIntegral ip_field) empty_user empty_ip) (EditIpCounts 0 0) traildb
 where
  countEditsAndIPs :: Cursor -> Int -> Int -> Feature -> Feature -> EditIpCounts -> TrailID -> IO EditIpCounts
  countEditsAndIPs cursor user_field ip_field empty_user empty_ip counts trail_id = do
    setCursor cursor trail_id

    let go accum@(EditIpCounts edits ip_edits) =
